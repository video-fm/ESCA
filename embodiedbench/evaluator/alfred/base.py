...
# os.environ["remote_url"] = "http://0.0.0.0:15238/v1"
# os.environ["DISPLAY"] = "cherry:11.0"
# os.environ["OPENAI_API_KEY"] = "sk-proj-klfjalfadsil;fkadsl;kfsjfkl"
# home_dir = "/home/asethi04/embody/EmbodiedBench"
# existing_path = os.environ.get("PYTHONPATH", "")
# os.environ["PYTHONPATH"] = f"{home_dir}:{existing_path}"
import numpy as np
...
from embodiedbench.evaluator.evaluator_utils import (
    load_saved_data,
...
exploration_example_path = os.path.join(
    os.path.dirname(__file__), "config/alfred_long_horizon_examples.json"
)
...
class EB_AlfredEvaluator:
    ...
    def check_config_valid(self):
        if self.config["multistep"] + self.config["chat_history"] > 1:
            raise ValueError(
                "Only one of multistep, chat_history can be enabled at a time."
            )
    ...
    def evaluate_main(self):
        ...
        for eval_set in valid_eval_sets:
            ...
            exp_name = (
                f"{self.model_name.split('/')[-1]}_{self.config['exp_name']}/{eval_set}"
                if len(self.config["exp_name"])
                else f"{self.model_name.split('/')[-1]}/{eval_set}"
            )
            self.env = EBAlfEnv(
            ...
            examples = (
                json.load(open(example_path, "r+"))
                if self.eval_set != "long_horizon"
                else json.load(open(exploration_example_path, "r+"))
            )
            ...
            self.evaluate()
            average_json_values(
                os.path.join(self.env.log_path, "results"), output_file="summary.json"
            )
    ...
    def evaluate(self):
        ...
        while self.env._current_episode_num < self.env.number_of_episodes:
            ...
            while not done:
                try:
                    ...
                    if action == -1:
                        ...
                        if (
                            self.env._cur_invalid_actions
                            >= self.env._max_invalid_actions
                        ):
                            break
                        continue
                    ...
                    if type(action) == list:
                        for action_single in action[
                            ...
                            obs, reward, done, info = self.env.step(
                                action_single, reasoning=reasoning
                            )
                            action_str = (
                                action_single
                                if type(action_single) == str
                                else self.env.language_skill_set[action_single]
                            )
                            print(
                                f"Executed action: {action_str}, Task success: {info['task_success']}"
                            )
                            ...
                            if done or not info["last_action_success"]:
                                # stop or replanning
                                print(
                                    "Invalid action or task complete. If invalid then Replanning."
                                )
                                break
                    else:  # single action
                        obs, reward, done, info = self.env.step(
            ...
            episode_info["num_invalid_action_ratio"] = (
                episode_info["num_invalid_actions"] / info["env_step"]
                if info["env_step"] > 0
                else 0
            )
...
if __name__ == "__main__":
    ...
    def parse_arguments():
        ...
        parser.add_argument(
            "--down_sample_ratio", type=float, help="Down sample ratio."
        )
        parser.add_argument("--model_type", type=str, help="Type of the model.")
        parser.add_argument(
            "--language_only", type=int, help="Set to True for language only mode."
        )
        parser.add_argument("--exp_name", type=str, help="Name of the experiment.")
        parser.add_argument(
            "--chat_history", type=int, help="Set to True to enable chat history."
        )
        parser.add_argument(
            "--detection_box", type=int, help="Set to True to enable detection."
        )
        parser.add_argument(
            "--eval_sets",
            type=lambda s: s.split(","),
            help="Comma-separated list of evaluation sets.",
        )
        parser.add_argument(
            "--selected_indexes",
            type=lambda s: [int(x) for x in s.split(",")],
            help="Comma-separated list of episode indices (0-indexed).",
        )
        parser.add_argument(
            "--multistep", type=int, help="Number of steps for multi-step reasoning."
        )
        parser.add_argument("--resolution", type=int, help="Resolution for processing.")
        parser.add_argument(
            "--env_feedback",
            type=int,
            help="Set to True to enable environment feedback.",
        )
        parser.add_argument(
            "--tp",
            type=int,
            help="number of tensor parallel splits of the model parameters",
        )
