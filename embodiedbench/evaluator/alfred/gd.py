...
from embodiedbench.planner.vlm_planner_gd import VLMPlanner
...
class EB_AlfredEvaluator():
    ...
    def evaluate_main(self):
        ...
        for eval_set in valid_eval_sets:
            ...
            self.planner = VLMPlanner(self.model_name, model_type, self.env.language_skill_set, system_prompt, examples, scene_graph_examples=scene_graph_examples, n_shot=self.config['n_shots'], 
                                            obs_key='head_rgb', chat_history=self.config['chat_history'], language_only=self.config['language_only'],
                                            use_feedback=self.config.get('env_feedback', True), multistep=self.config.get('multistep', 0), tp=self.config.get('tp', 1), gd_checkpoint_path=self.gd_checkpoint_path, gd_config_path=self.gd_config_path)
...
if __name__ == '__main__':
    ...
    def parse_arguments():
        ...
        parser.add_argument('--model_name', default='gpt-4o', type=str, help='Name of the model.')
        ...
        parser.add_argument('--exp_name', default='FINAL_gpt_gd', type=str, help='Name of the experiment.')
    ...
    config = {
        ...
        'language_only': 0,
        'exp_name': 'FINAL_gpt_gd',
        'chat_history': 0, 
        'detection_box': 1,
        'eval_sets': ['base', 'common_sense'],
        # 'base', 'common_sense', 'complex_instruction', 'spatial', 'visual_appearance', 'long_horizon' 
        'selected_indexes': [], 
        'multistep':0, 
        'resolution': 500, 
        'env_feedback': 1,
