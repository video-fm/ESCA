...
class EB_NavigationEvaluator():
    ...
    def evaluate_video(self):
        ...
        while self.env._current_episode_num < self.env.number_of_episodes:
            ...
            obs = self.env.reset()
            img_path = self.env.save_image(obs)
            video_path = img_path[:-4] + '.mp4'
            frames = [obs['head_rgb']]
            
            save_video_from_rgb_images(frames, video_path)
            ...
            while not done:
                # try:
                    video_path = img_path[:-4] + '.mp4'
                    save_video_from_rgb_images(frames, video_path)
                    action, reasoning = self.planner.act_video(video_path, user_instruction)
                    frames = []
                    print(f"Planner Output Action: {action}")
                    reasoning = json.loads(reasoning)
                    if type(action) == list:
...
if __name__ == '__main__':
    ...
    config = {'model_name': 'gpt-4o', 
              ...
              'model_type': 'remote', 
              'language_only': False, 
            #   'eval_sets': ['common_sense', 'base',  'visual_appearance', 'complex_instruction', 'long_horizon'], 
